...
if [  `"${mysql_bin_path}/bin/mysql_config_editor" print -G${port}|wc -l` -gt 1 ] ;then 
	...
	#read choice yh alter this for CI#############sd
	choice="S"
...
if [ -d "${mysql_software_prefix}/${mysql_software_version}" ] ; then 
	...
	do
		...
		skip_mysql_software="R"
		#read skip_mysql_software yh alter this place for CI#########
	done;
...
#read r yh alter this place for CI#########
r="Y"
...
#read confirm yh alter this place for CI#########
confirm="Y"
...
mv "${znvdata_version}"  "$mysql_data_path/../"
...
for db in dcvs  dcvs_auth  dcvs_apinetwork 
do
